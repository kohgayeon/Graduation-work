...
class CNN_LSTM(nn.Module):
    def __init__(self):
        ...
        self.lstm1 = nn.LSTM(input_size=500, hidden_size=250, num_layers=1, batch_first=True)
        self.lstm2 = nn.LSTM(input_size=250, hidden_size=50, num_layers=1, batch_first=True)
    ...
    def forward(self, x):
        ...
        x = F.relu(self.fc1(x))
        x = self.dropout(x)
        x = x.view(x.size(0), -1, 500)  # reshape to [batch_size, seq_len, input_size] for LSTM

        # pass through LSTM layers
        h01 = torch.zeros(1, x.size(0), 250).to(x.device)
        c01 = torch.zeros(1, x.size(0), 250).to(x.device)
        out1, _ = self.lstm1(x, (h01, c01))

        h02 = torch.zeros(1, x.size(0), 50).to(x.device)
        c02 = torch.zeros(1, x.size(0), 50).to(x.device)
